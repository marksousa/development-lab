...
def client_input(msg = "What would you like? (espresso/latte/cappuccino):"):
    ...
    if client_choice in MENU.keys(): return selling_flow(MENU.get(client_choice))
...
def selling_flow(client_choice):
    ...
    if client_choice :
        ...
        if client_wallet >= int(client_choice["cost"]):
            print("Ok.")
            print(client_choice)
        else :
            print("Not enough.")
...
while machine_on:
    drink_available = client_input()
    selling_flow(drink_available)